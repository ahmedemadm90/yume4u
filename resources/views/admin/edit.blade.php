...
<div class="app-content content">
    <div class="content-wrapper">
        ...
        <div class="col-md-12 col-xl-12">
            <div class="card">
                <h5 class="card-header">{{ trans('تعديل بياناتي') }}</h5>
                <form action="{{route('admin.update.myinfo',$admin->id)}}" enctype="multipart/form-data" method="POST">
                    @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select id="role_id" class="js-example-basic-single form-control form-control-select2"
                                name="role_id">
                                <option selected>{{ trans('Choose Role') }}</option>
                                @foreach ($roles as $role)
                                @if($role->id < 4) <option value="{{ $role->id }}" @if($role->id == $admin -> role_id)
                                    selected @endif>
                                    {{ $role->name }}</option>
                                    @endif
                                    @endforeach
                        ...
                        <div class="form-group">
                            <label for="password">{{ trans('تأكيد كلمة المرور') }}</label>
                            <input type="text" name="password" class="form-control" id="confirm-password"
